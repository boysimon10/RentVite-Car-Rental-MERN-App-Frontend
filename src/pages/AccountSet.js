...
function AccountSet() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${process.env.REACT_APP_API_URL}user/${uid}/profile-picture`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const toggleRole = async () => {
    try {
      ...
      if (user.role === "business") {
        alert(
          "L'utilisateur ne peut pas repasser en compte client. Contacter le service client"
        );
        return;
      }
                ...
                <div className="mt-8 lg:w-1/2 lg:mt-0">
                  <form className="w-full lg:max-w-xl">
                    ...
                    <div className="w-full mx-auto my-11">
                      <label className="text-xl text-black mb-2 block">
                        {isRoleBusiness
                          ? "Vous avez un compte Business"
                          : "Passer en Business pour mettre en location vos voitures (Irréversible)"}
                      </label>
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={isRoleBusiness}
                          onChange={toggleRole}
                          defaultValue=""
